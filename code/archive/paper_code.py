...
# remove depression data from dataset
data = data[data['class'] != "depression"].reset_index()

groups = data['user']

# Features and labels
dataX = data.copy().drop(['class', 'user'],axis=1)
...
# split into sets
X_train, X_test, y_train, y_test, groups_train, groups_test = train_test_split(dataX,
                                    dataY,groups , test_size=0.10,
...
k_fold = StratifiedGroupKFold(n_splits=10,shuffle=True,random_state=2018)
...
# print("Samples:", len(dataX))

# checking for leakage because accuracy scores are too high
y_shuffled = pd.Series(y_train).copy()
for user in np.unique(groups_train):
    idx = groups_train == user
    y_shuffled[idx] = np.random.permutation(y_shuffled[idx])

# Cross-validation
trainingScores = []
cvScores = []
predictionsBasedOnKFolds = pd.DataFrame(data=[],columns=[0,1])
# predictionsBasedOnKFolds = pd.DataFrame(data=[],
                                        # index=y_train.index,columns=[0,1])
model = logReg
scores = []

for train_index, cv_index in k_fold.split(X_train, y_shuffled, groups_train):
# for train_index, cv_index in k_fold.split(np.zeros(len(X_train)),y_train.ravel()):
    X_train_fold, X_cv_fold = X_train[train_index,:], X_train[cv_index,:]
    y_train_fold, y_cv_fold = y_shuffled[train_index], y_shuffled[cv_index]
    ...
    print(np.bincount(y_cv_fold))
    print(data.groupby("user")["class"].value_counts())

    # predictionsBasedOnKFolds.loc[X_cv_fold.index,:] = model.predict_proba(X_cv_fold)
    predictions = model.predict_proba(X_cv_fold)[:,1]
    loglossCV = log_loss(y_cv_fold, predictions)
    cvScores.append(loglossCV)
    average_precision = average_precision_score(y_cv_fold, predictions)
...
for train_index, cv_index in k_fold.split(np.zeros(len(X_train)), y_train.ravel()):
    X_train_fold, X_cv_fold = X_trainWithPredictions[train_index,:], X_trainWithPredictions[cv_index,:]
    y_train_fold, y_cv_fold = y_train[train_index], y_train[cv_index]

    lgb_train = lgb.Dataset(X_train_fold, y_train_fold)
    lgb_eval = lgb.Dataset(X_cv_fold, y_cv_fold, reference=lgb_train)
    gbm = lgb.train(params_lightGB_ensemble, lgb_train, num_boost_round=2000,
                    valid_sets=lgb_eval, early_stopping_rounds=200)
